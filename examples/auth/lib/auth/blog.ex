defmodule Auth.Blog do
  ...
  alias Auth.Blog.Post
  alias Auth.Accounts.Scope
  ...
  def subscribe_posts(%Scope{} = scope) do
    ...
    Phoenix.PubSub.subscribe(Auth.PubSub, "user:#{key}:posts")
  end

  defp broadcast_post(%Scope{} = scope, message) do
    key = scope.user.id

    Phoenix.PubSub.broadcast(Auth.PubSub, "user:#{key}:posts", message)
  end
